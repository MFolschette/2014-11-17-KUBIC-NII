...
\begin{tikzpicture}[adn]
  ...
  \path<2->
    node[alabel, above=-3pt of a] {$\segm{0}{1}$}
    node[alabel, below=-3pt of b] {$\segm{0}{1}$}
    node[alabel, below=-3pt of z] {$\segm{0}{2}$};
  \path<3->
  ...
  \path<4->
    ...
    (b) edge[draw=none,bend right=15] node[alabel,right=+1pt] {$-1$} (a)
    (b) edge[draw=none,loop left] node[alabel,left=+2pt] {$+1$} (b)
    (a) edge[draw=none] node[alabel,above=+1pt] {$+1$} (z)
    (b) edge[draw=none] node[alabel,below=+1pt] {$+1$} (z);
\end{tikzpicture}
\end{center}
